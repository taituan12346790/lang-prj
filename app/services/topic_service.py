...
def _topic_to_response(topic: Topic, prog: Optional[UserTopicProgress]) -> TopicResponse:
    return TopicResponse(
        ...
        lesson_count=len(topic.lessons) if topic.lessons else 4,
...
class TopicService:
    ...
    async def submit_quiz(
        ...
        """Grade quiz, update progress, return detailed results."""
        # Get quiz lesson
        result = await db.execute(
            select(Lesson).where(
                Lesson.topic_id == topic_id,
        ...
        # Feedback message
        if score == 100:
            feedback = "🏆 Xuất sắc! Bạn đạt điểm tuyệt đối!"
        elif score >= 80:
            feedback = "🎉 Rất tốt! Bạn đã nắm vững chủ đề này!"
        elif score >= 70:
            feedback = "✅ Đạt! Tiếp tục cố gắng nhé!"
        else:
            feedback = "📚 Chưa đạt. Hãy xem lại bài học và thử lại!"
        ...
        if not prog:
            prog = UserTopicProgress(
                ...
                status="in_progress", lesson_completed=4,
            ...
            prog.lesson_completed = max(prog.lesson_completed or 0, 4)
